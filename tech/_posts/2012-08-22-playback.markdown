...
It also had multiple features we were looking for in terms of front-end development, namely `compiled _CoffeeScript_, routes, and templates`. Something I think is especially important to note: `do not use Scala templates to render data to the page`; it makes it much harder to manipulate.
...
<h2>Functionality</h2>
The application does a few things that seem to be standard across any web application, but standardizes a workflow using a concept of _Pages_ **\[more on that later\]**. It starts with _two_ arbitrarily named pages; we all know how quickly what needs to show up first on a dashboard can change in development cycles. The first page displays a _Collection_ in `table` form. _Models_ can be added to the _Collection_, or removed from it, and DOM elements are manipulated in the process. The second page displays a _Collection_ in `div` form, and updates when a WebSocket server running locally **\[a message is triggered by button-click\]**, pushes a new _Model_ to the page.
...
The `main.scala.html` reference holds all references to the third-party libraries we are using to make the application browser compatible back to _Internet Explorer 7_; there's nothing like `building on the shoulders of giants`:
...
<h2>Logging</h2>
By appending a _GET_ parameter of `?log=true`, we have the ability to urn on logging to the application, and it's _Internet Explorer_ safe! `By default, logging is turned off`.

For application logging, I got tired of writing `debug.info 'MyClass.myMethod()` [really quickly](http://stackoverflow.com/questions/11973181/log-every-method). The solution is to prepend every method but `constructor` with `@trace` to make `debug.info` happen on every method automatically. **Note:** one minor issue is that `App.SomeSpace.MyClass` logs as `MyClass`.
...
1. Using some Googled trickery, routes are not case sensitive!
...
1. _Pages_ are generic references to actions, as when developing an application, the page functionality - and it's name - might change on completion.
...
1. Templates have `obj.someId` to avoid breaking when an attribute is undefined, even if it prints undefined.
1. All _Pages_ have their own CSS files and classes inside them are prefixed based on their name; this makes it easier to track down, at a glance, issues.
...
1. Declare routes.
1. Instantiate application classes **[\these are equivalent to _Pages_ \]**.
1. Geo-locate.
...
1. `/app/assets/javscripts/pages/action3/events.coffee`

1. `/app/assets/javscripts/pages/action3/state.coffee`

1. `/app/assets/javscripts/pages/action3/ui.coffee`

1. `/app/assets/javscripts/pages/action3/view.coffee`

In order to simplify pages, I'm using the `eval()` function to instantiate _Page_ classes from a single defined route object inside `/app/assets/javscripts/app/app.coffee` **\[view the source of the Routes class, it's pretty cool\**]/. We need to a reference to our new route:
...
The `doSomething` method references a method of the same name in the _Events_ class related to the page; `it's a good idea to keep all logic inside the _Events_ classes of pages`.

<h2>Events</h2>
Since _Page_ logic is contained the _Events_ classes, they can grow to be quite large. Here's a brief overview of what I see happening most of the time on _Page_ render:

1. Check if something is defined, and it's possibly kept in global state.
1. If not defined, go somewhere else.
1. If defined, request some data, render it to the _Page_.

In the code, this boils down to:

1. Check if something is defined in `continueToLoad`, return `false` if it's not, and reference `App.Router.navigate toMyPage` if ti's not.
...
That's all she wrote for now. I didn't go through the _Helper_ classes much, as the code for those is well-documented and self-explanatory. I'll work on making this a more exhaustive tutorial.