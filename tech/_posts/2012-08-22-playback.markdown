...
<h2>Standards</h2>
1. All CoffeeScript, all the time.
...
1. No `""`, only `''` in Javascripts, or no double-quotes. 

<h2>Open Source</h2>
I made the decision to open-source this architecture for a few reasons:

1. I haven't found anything like this in the wild.
1. It's a good, real-world use case of an application in production.
1. I love open-source, and want the community to help `make it better` with their knowledge.

<h2>But really, TL;DR;</h2>
The application does a mariade of things which seem to be prevalent in any standard application. It has _two_ pages, the first of which displays a collection, held in page state, in table form. The collection can be modified through table interaction. The second page displays a collection held in global state, which updates on WebSocket requests (for this demo, it's just an echo-server which is simulated by clicking the button). jQuery is used to manipulate the states of various dialogue notifications and display different shit when the collections are empty.

There are some generic helpers that do modifications to collections, as well as handle browser compatibility. 
...
<h2>IDE</h2>
Import the project into your editor of choice; it's _Eclipse_ in my case. The _Scala_ plugin and _Play!_ do not always play well. Some things to be aware of:

1. When adding new template files, you have to specify `Ok` in the controller, `play run` the application, then `refresh` the _Eclipse_ project to be able to reference the newly added template. It's a bit silly.

1. Lots of CoffeeScript files slow down compiling the app when packaged Javascripts are changed, and it's quite noticeable. I'm not sure why they haven't made this better. I've even tried an alternative compiler to no avail.
...
<h2>Routing</h2>
Rendering a single HTML file, which represents our entire _Backbone.js_ application, will require _two_ modifications to _Play!_ files. Normally, we'd have to add a route to the `/conf/routes` file to point the HTTP request to the right place, but it's already done for us: 
...
Using Play!'s Scala templates, we have the ability to manage our assets in one place. These templates require a `.scala` prefix on any HTML file. It makes it really easy to pass data from the server side to the templates, but I only use them for resources **\[I'm trying to be as framework-agnostic as possible\]**. Think of them as something similar to [Mustache](mustache.github.com) or [Underscore](http://underscore.org).

Create a new template file in `/app/views/` named `app.scala.html`. This is our the main application template, but is strictly a wireframe for passing resources to another template where we'll hold all our static resources, `main.scala.html` **\[a Play! standard\]**. `By doing this we can define separate applications`, if we like, inside a single Play! application, **\[think PROD, STAGING, and DEV\]**. There isn't a way to share client-side resources between _two_ separate Play! apps like there is on the server-side **\[vis-a-vis [plugins](https://github.com/playframework/Play20/wiki/Modules)\]**, and creating separate applications without using separate `git branches` seemed to work well like this. The whole file is displayed below, and references every non third-party Javascript we're using:
...
&lt;head&gt;
   ...
   &lt;link rel=&quot;stylesheet&quot; href=&quot;/public/stylesheets/third-party/bootstrap.min.css&quot;&gt;
   &lt;link rel=&quot;stylesheet&quot; href=&quot;/public/jstylesheets/third-party/bootstrap-responsive.min.css&quot;&gt;
   &lt;link rel=&quot;stylesheet&quot; href=&quot;/public/stylesheets/third-party/prettify.css&quot;&gt;
...
Normally, an app might have various `Regions` associated wiht it - a menu, a top bar (bootstrappy). That's Ok. Those are pages in themselves, and the only difference between adding a standard page and adding another region to a layout is that you don't specify the route the same. Instead, in the Layout `regions` object, you add the reference to the template.

<h2>templates</h2>
obj.data so as to avoid null valuds

even if verbose, i got away from sharing templates. most of the time, css tweaks made them different. so, each page ended up having a similar refercne to a collection, if not just styled differently. 
...
This is always done so I can call something.data, instead of something.get 'data' ...normal?

<h2>collections with marionette</h2>
I consitently ran into the issue of displaying an empty collection, and formed a pattern of showing/hiding. Whether or not it works for everyone is the issue.
